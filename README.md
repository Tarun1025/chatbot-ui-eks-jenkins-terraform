...
Before proceeding to next steps. Do the following things.
...
Do some modifications to the backend.tf file such as changing the **bucket** name and **DynamoDB** table.

![](<https://miro.medium.com/v2/resize:fit:700/1*ts7vnOdjjkq38TSgWD0hCA.png>)

**Configure AWS CLI:** Run the below command, and add your access keys.

```go
aws configure
```

You have to replace the Pem File name with one that is already created on AWS.
...
Upon success,this will create an ec2 server with name “Jenkins-server”.
